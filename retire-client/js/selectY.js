layui.define('layer',function (exports) {
    ...
    var P = function(o){
        ...
        //     return tree;
        // };
        this.getChild = function(data,code_superior){
            var child = [];
			//child.push(data[0]);
            for (var i = 0; i < data.length; i++) {
                if (data[i].code_superior == code_superior) {
        ...
        this.createChild = function(items){
            ...
            for(var i = 0; i < items.length; i++){
                var row = o.getChild(o.data,items[i].value);
                row = row.length != 0 ? 'child-row' : '';
                var off = items[i].off ? 'layui-disabled' : '';
                ul += '<li class ="'+row+' '+off+'" data-id="'+items[i].value+'">'+items[i].label+'</li>'
            }
            ul += '</ul>';
            return ul;
        };
        this.setValue = function (value) {
            ...
            for(var i=0;i<data.length;i++){
                for(var j=0;j<value.length;j++){
                    if(value[j] == data[i].label){
                        //data[i].checked = true;
                        itemID.push(data[i].value);
                    }
                }
            }
            return itemID;
        };
        this.showPopup = function (data,value) {
            var o =this;
            var items = o.getChild(data,value);
            if(items.length != 0){
