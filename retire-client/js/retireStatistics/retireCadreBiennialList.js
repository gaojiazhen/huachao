layui.use(['form','layer','table','laytpl','common'],function(){	
	...
	function Action(){
		...
		common.ajax(listBaseDepartment, 'post', 'json', unitParam, function (dres) {
			...
			common.ajax(listRetireCadreBiennialChange, 'post', 'json', userParam, function (ures) {
				var tableIns = table.render({
					...
					cols : [[
						...
						{field: 'PREVIOUS_YEAR_CADRE', title: '上年12月底<br>退休干部总数', width:130,minWidth:130, align:"center", rowspan:2},
						{field: 'THIS_YEAR_RETIRE', title: '本年度内办理<br>退休手续干部数', width:130,minWidth:130, align:"center", rowspan:2},
						{field: 'THIS_YEAR_DECEASED', title: '本年度内<br>去世退休干部数', width:130,minWidth:130, align:"center", rowspan:2},
						{title: '本年12月底<br>应有退休干部数', width:130,minWidth:130, align:"center", rowspan:2, templet: function(d){
							return Number(d.PREVIOUS_YEAR_CADRE) + Number(d.THIS_YEAR_RETIRE) - Number(d.THIS_YEAR_DECEASED);
						}},
						{title: '本年12月底<br>实有退休干部数', width:130,minWidth:130, align:"center", rowspan:2, templet: function(d){
							return Number(d.PREVIOUS_YEAR_CADRE) + Number(d.THIS_YEAR_RETIRE) - Number(d.THIS_YEAR_DECEASED);
						}},
	...
	$("#excel").click(function(){
	    $("#formid").attr("action",excelRetireCadreBiennialChange).trigger("submit");
	});
})