...
layui.define(["element","jquery"],function(exports){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                if(data[i].icon != undefined && data[i].icon != ''){
                    ulHtml += '<a class="layui-icon '+data[i].icon+'" data-icon="'+data[i].icon+'">';
                }else{
					ulHtml += '<a>';
				}
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if(data[i].children[j].icon != undefined && data[i].children[j].icon != ''){
                        ulHtml += ' class="layui-icon '+data[i].children[j].icon+'" data-icon="'+data[i].children[j].icon+'">';
                    }else{
						ulHtml += ' >';
					}
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                    ulHtml += ' class="layui-icon '+data[i].icon+'" data-icon="'+data[i].icon;
                }else{
					ulHtml += ' >';
				}
	...
	Tab.prototype.render = function() {
		...
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="page/main.html"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height(windowHeight);
		//兼容IE8
		$(".navBar").attr("style","position:absolute; height:"+windowHeight+"; overflow:auto");
